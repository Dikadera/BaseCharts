// ══════════════════════════════════════════════════════════════
// ChartsonBase — Full Application Script
...
// Add helper: switch the user's wallet to Base Sepolia (adds chain if missing)
async function switchToBaseSepolia(provider = window.ethereum) {
  const defaultRpc = 'https://sepolia.base.org';
  const defaultChainId = '0x14a34';
...
async function checkConfigStatus() {
  try {
    ...
    if (data.configured) {
      ...
      modeText.textContent = 'Live API (Base Network)';
    } else {
...
async function connectWalletWith(provider) {
  ...
  try {
    ...
    const eip = {
      ...
      domain: { name: 'ChartsonBase Portal', version: '1.0', chainId: 84532, verifyingContract: '0x036CbD53842c5426634e7929541eC2318f3dCF7e' },
      primaryType: 'SignIn',
      message: { contents: `Authenticate with ChartsonBase via ${WALLET_NAMES[provider]}. Session expires in 24 hours.`, nonce: String(Date.now()) }
    };
...
function disconnectWallet() {
  ...
  // Reset header button
  document.getElementById('wallet-btn-text').textContent = 'Connect Base Wallet';
...
document.getElementById('btn-sign-message').addEventListener('click', async () => {
  ...
  try {
    ...
    if (!isCircleArc) {
      // Client-side signing via MetaMask/Phantom/Coinbase
      ...
      // Log client-side signature
      const clientLog = {
        timestamp: new Date().toISOString(),
        endpoint: 'eth_signTypedData_v4',
        method: 'PROVIDER',
...
async function subscribeToPlan(planKey) {
  ...
  try {
    ...
    if (!isCircleArc) {
      ...
      // Ensure wallet is on Base Sepolia (adds the chain if missing)
      ...
      const tokenContract = (state.config && state.config.details && state.config.details.usdcTokenId) || '0x036CbD53842c5426634e7929541eC2318f3dCF7e';

      // amount in USDC has 6 decimals on Base Sepolia
...
document.getElementById('btn-buy-more-lives')?.addEventListener('click', () => scrollToPlans());
document.getElementById('btn-upgrade-plan')?.addEventListener('click', () => scrollToPlans());
