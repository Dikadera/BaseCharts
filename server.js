...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", "https://api.binance.com", "https://data-api.binance.vision", "https://api1.binance.com", "https://api2.binance.com", "https://api.circle.com", "https://generativelanguage.googleapis.com"]
    }
  }
}));
app.use(compression());
...
async function fetchCandlesForPair(pair, timeframe = '1H') {
  ...
  if (pair.type === 'crypto') {
    try {
      ...
      console.error(`Failed to fetch Binance klines for ${pair.symbol}, falling back to mock data:`, err.message);
    }
  }
...
// ─────────────────────────────────────────────────────────
// API: /api/pair-data  (quota-enforced chart data endpoint)
// ─────────────────────────────────────────────────────────
app.get('/api/pair-data', async (req, res) => {
  ...
  if (address) {
    ...
    if (!user) {
      user = { walletAddress: address, plan: null, pairsAllowed: 0, pairsUsed: 0, pairsAccessed: [], transactions: [] };
    ...
    if (!isAlreadyAccessed) {
      if (user.pairsAccessed.length >= user.pairsAllowed) {
        return res.status(403).json({
          ...
          message: `Live chart quota reached for your ${user.plan} plan. You've used all ${user.pairsAllowed} live pairs. Buy more credits or upgrade your plan.`
        });
      }
  ...
  try {
    ...
    console.error('pair-data error:', err.message);
...
app.post('/api/auth/sign', async (req, res) => {
  ...
  if (isConfigured && typedData && typedData.domain) {
    typedData.domain.chainId = 84532;
  }

  const requestPayload = { walletAddress, blockchain: 'BASE-SEPOLIA', eip712Domain: typedData?.domain };
...
app.post('/api/payment/transfer', async (req, res) => {
  ...
  const destinationWallet = process.env.CIRCLE_DESTINATION_WALLET_ID || '0xSimulatedMerchantWalletAddressBaseL2';
  ...
  const requestPayload = {
    ...
    feeLevel: 'LOW',
    tokenId: process.env.CIRCLE_USDC_TOKEN_ID || '5a64c456-03f7-5238-a77a-a780b0b90263'
  };
...
// Supports both MetaMask on-chain and Circle Base backend
// ─────────────────────────────────────────────────────────
app.post('/api/payment/subscribe', async (req, res) => {
  ...
  if (isCircleArc) {
    // Circle Base Developer Controlled Wallets Transfer
...
if (require.main === module) {
  app.listen(PORT, () => {
    console.log(`================================================================`);
    console.log(` 🌀 ChartsonBase v2.0 running on http://localhost:${PORT}`);
